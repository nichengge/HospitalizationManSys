...
function Query(){
	...
	var data = {"startTime":startTimes,"endTime":endTimes,"type":event_type};
	$.ajax({
...
function showList(list,start,end){
	if(list.length>0){
		for(var i=start;i<end;i++){
			...
			var $tr = $(trStyle+
				        "<td class='node'>"+log.id+"</td>"+
				        "<td class='time'>"+log.type+"</td>"+
				        "<td class='name'>"+log.create_time+"</td>"+
				        "<td class='name'>"+logdetial+"</td>"+
			...
			if(result2.state==0){
				...
				if(onelog.length>0){
					...
					showOneLog(id,type,create_time,detial);
						...
						/**
						 * 因为每隔3天的凌晨12点执行清log库，
						 * 出现这个原因肯定是请求的前一秒在12点以前，收到请求以后超过了12点
						 * */
