...
function showbar(lists){
	...
	var option1 = {
			title : {text: '各科室病床使用情况统计图',subtext: '床位总数 = 未使用 + 已使用',x:'center'},
		    tooltip : {trigger: 'axis',axisPointer : {type : 'shadow'}},
		    legend: {data:['未使用', '已使用', '床位总数'],right: 'right'},
		    grid: {left: '3%',right: '4%',bottom: '3%',containLabel: true},
		    xAxis : [{type : 'value'}],
		    yAxis : [{type : 'category',axisTick : {show: false},data:[]}],
		    series : [{name:'未使用',type:'bar',label: {normal: {show: true,position: 'inside'}},data:[]},
		    		  {name:'已使用',type:'bar',stack: '总量',label: {normal: {show: true,position: 'left'}},data:[]},
		        	  {name:'床位总数',type:'bar',stack: '总量',label: {normal: {show: true}},data:[]}
		        	  ]
					};
	//动态生成纵坐标科室名称数据
...
function showpie(lists){
	...
	var option2 = {
		    title : {text: '各科室病床使用率(%)',subtext: '科室病床使用率 = (已使用/科室床位总数)*100%',x:'center'},
		    tooltip : {trigger: 'item',formatter: "{a} <br/>{b} : {c} ({d}%)"},
		    legend: {data: ['已使用','未使用'],right:'right'},
	...
	/*
	 * 如果只选择了一个科室，则显示当前选择的科室的病床使用率
	 * 如果选择了全部，则显示全部可是的病床使用率
	 * */
	if(lists.length == 1){
		//装载科室名称
	    ...
	    var used = {value:use, name:'已使用'}//已使用
	    var unuse = {value:lists[0].wy, name:'未使用'}//未使用
	    ...
	    var used = {value:sumused, name:'已使用'}//已使用
	    var unuse = {value:sumwy, name:'未使用'}//未使用
	    option2.series[0].data.push(used);
	    option2.series[0].data.push(unuse);	
	}
    //console.log(option2);
